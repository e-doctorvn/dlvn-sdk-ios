// swift-tools-version: 5.8
// The swift-tools-version declares the minimum version of Swift required to build this package.

import PackageDescription

let package = Package(
    name: "EdoctorDlvnSdk",
    platforms: [
        .macOS(.v10_14), .iOS(.v11)
    ],
    products: [
        // Products define the executables and libraries a package produces, and make them visible to other packages.
        .library(
            name: "EdoctorDlvnSdk",
            targets: ["EdoctorDlvnSdk"]),
    ],
    dependencies: [
        // Dependencies declare other packages that this package depends on.
        // .package(url: /* package url */, from: "1.0.0"),
        .package(url: "https://github.com/sendbird/sendbird-calls-ios.git", exact: "1.10.17"),
        .package(url: "https://github.com/sendbird/sendbird-chat-sdk-ios.git", exact: "4.15.1"),
//        .package(url: "https://github.com/firebase/firebase-ios-sdk.git", from: "10.24.0"),
//        .package(url: "https://github.com/apollographql/apollo-ios.git", from: "0.36.0"),
//        .package(path: "./API")
    ],
    targets: [
        // Targets are the basic building blocks of a package. A target can define a module or a test suite.
        // Targets can depend on other targets in this package, and on products in packages this package depends on.
        .target(
            name: "EdoctorDlvnSdk",
            dependencies: [
                .product(name: "SendBirdCalls", package: "sendbird-calls-ios"),
                .product(name: "SendbirdChatSDK", package: "sendbird-chat-sdk-ios"),
//                .product(name: "FirebaseAnalytics", package: "firebase-ios-sdk"),
//                .product(name: "Apollo", package: "apollo-ios"),
//////                .product(name: "ApolloWebSocket", package: "apollo-ios-websocket"),
//                .product(name: "API", package: "API"),
            ]),
        .testTarget(
            name: "EdoctorDlvnSdkTests",
            dependencies: [
                .product(name: "SendBirdCalls", package: "sendbird-calls-ios"),
                .product(name: "SendbirdChatSDK", package: "sendbird-chat-sdk-ios"),
//                .product(name: "Apollo", package: "apollo-ios"),
//
//                .product(name: "API", package: "API"),
            ]),
    ]
)
